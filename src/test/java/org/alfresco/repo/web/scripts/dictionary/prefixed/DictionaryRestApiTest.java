...
public class DictionaryRestApiTest extends BaseWebScriptTest
{
    ...
    private void validateChildAssociation(JsonNode result) throws Exception
    {
        ...
        assertEquals("wca:form", result.get("source").get("class"));
        assertEquals(false, result.get("source").get("mandatory"));
        assertEquals(false, result.get("source").get("many"));
        
        assertEquals("wca:workflowdefaults", result.get("target").get("class"));
    ...
    private void validateAssociation(JsonNode result) throws Exception
    {
        ...
        assertEquals("wca:form", result.get("source").get("class"));
        assertEquals("wca:capture", result.get("source").get("role"));
        assertEquals(false, result.get("source").get("mandatory"));
        assertEquals(false, result.get("source").get("many"));
        
        assertEquals("wca:renderingenginetemplate", result.get("target").get("class"));
        assertEquals("wca:presentation", result.get("target").get("role"));
        assertEquals(false, result.get("target").get("mandatory"));
    ...
    private void validateTypeClass(JsonNode result) throws Exception
    {
        ...
        assertEquals("/api/defclasses/sys/base", result.get("parent").get("url"));
        ...
        assertEquals("/api/defclasses/cm/cmobject/property/sys/referenceable", result.get("defaultAspects").get("sys:referenceable").get("url"));
        ...
        assertEquals("/api/defclasses/cm/cmobject/property/cm/auditable", result.get("defaultAspects").get("cm:auditable").get("url"));
        ...
        //assertEquals("/api/defclasses/cm/cmobject/property/cm/name", result.get("properties").get("cm:name").get("url"));
    ...
    private void validateAspectClass(JsonNode result) throws Exception
    {
        ...
        if (result.get("properties").has("cm:automaticUpdate") == true)
        {
            ...
            assertEquals("/api/defclasses/cm/thumbnailed/property/cm/automaticUpdate", result.get("properties").get("cm:automaticUpdate").get("url"));
        }
    ...
    private void validatePropertiesConformity(ArrayNode classDefs) throws Exception
    {
        ...
        for (int i = 0; (i < itemsToTest) && (i < classDefs.size()); ++i)
        {
            ...
            {
                propertyValues1 = new ArrayList<>(propertyNames1.size());
                for(JsonNode node: propertyNames1)
                {
                    propertyValues1.add(node.textValue());
                }
            }
            ...
            JsonNode classDef2 = AlfrescoDefaultObjectMapper.getReader().readTree(responseFromGetClassDef.getContentAsString());
            ...
            {
                propertyValues2 = new ArrayList<>(propertyNames2.size());
                for(JsonNode node: propertyNames2)
                {
                    propertyValues2.add(node.textValue());
                }
            }

            Response responseFromGetPropertiesDef = sendRequest(new GetRequest(classUrl + "/properties"), 200);
            ArrayNode propertiesDefs = (ArrayNode) AlfrescoDefaultObjectMapper.getReader().readTree(responseFromGetPropertiesDef.getContentAsString());
            assertEquals(200, responseFromGetClassDef.getStatus());
            // properties of class obtained by api/defclasses/class/properties
            List<String> propertyValues3 = new ArrayList<String>(propertiesDefs.size());
    ...
    public void testGetPropertyDefs() throws Exception
    {
        ...
        for (int i = 0; i < result.size(); i++)
        {
            ...
            System.out.println(title + " - " + result.get(i).get("name"));
      }
...
//        }
    ...
    public void testGetAssociationDefs() throws Exception
    {
        ...
        for(int i=0; i<result.size(); i++)
        {
            if(result.get(i).get("name").equals("wca:formworkflowdefaults")) 
                validateChildAssociation(result.get(i));
        }
        for(int i=0; i<result.size(); i++)
        {
            if(result.get(i).get("name").equals("wca:renderingenginetemplates")) 
